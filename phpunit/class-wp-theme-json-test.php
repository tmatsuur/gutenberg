...
class WP_Theme_JSON_Gutenberg_Test extends WP_UnitTestCase {
	...
	public function test_get_block_background_image_styles() {
		...
		$this->assertSame( $quote_styles, $theme_json->get_styles_for_block( $quote_node ), 'Styles returned from "::get_styles_for_block()" with core/quote background styles do not match expectations' );
		...
		$verse_styles = ":root :where(.wp-block-verse){background-image: url('http://example.org/top.png');background-position: 10% 20%;background-repeat: repeat;background-size: contain;background-attachment: scroll;}";
		$this->assertSame( $verse_styles, $theme_json->get_styles_for_block( $verse_node ), 'Styles returned from "::get_styles_for_block()" with core/verse background ref styles as string type do not match expectations' );
	}
